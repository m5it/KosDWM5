...
def find_kosdwm_source():
    ...
    for path in check_paths:
        if (path / "run.py").exists() or (path / "src" / "kosdwm.py").exists() or (path / "src" / "gadgets.py").exists():
            return path
    
    # Check current directory
    cwd = Path.cwd().resolve()
    if (cwd / "run.py").exists() or (cwd / "src" / "kosdwm.py").exists():
        return cwd
    ...
    for path in common:
        if (path / "run.py").exists() or (path / "src" / "kosdwm.py").exists():
            return path
...
def main():
    ...
    if not ((source / "run.py").exists() or (source / "src" / "kosdwm.py").exists()):
